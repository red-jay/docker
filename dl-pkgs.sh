...
for pkg in xen-system-amd64 libvirt-bin debootstrap virtinst libguestfs-tools systemd-sysv pinentry-tty rrdtool nut lm-sensors tmux vlan dnsmasq firewalld dhcpcd5 virtinst vncsnapshot tshark ; do
  # the very ugly grep filter removes virtual packages
  # we want word splitting in the below line
  # shellcheck disable=SC2046
  apt-get download $(apt-rdepends "${pkg}" | grep -v "^ " | grep -vE 'debconf-2.0|file-rc|perlapi-*|linux-initramfs-tool|awk|cron-daemon|sysvinit|pinentry')
done
